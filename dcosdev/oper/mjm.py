template = """
{
  ...
  "instances": 1,
  "user": "{{service.user}}",
  "cmd": "mv svc.yml engine-scheduler/svc.yml && export LD_LIBRARY_PATH=$MESOS_SANDBOX/libmesos-bundle/lib:$LD_LIBRARY_PATH; export MESOS_NATIVE_JAVA_LIBRARY=$(ls $MESOS_SANDBOX/libmesos-bundle/lib/libmesos-*.so); export JAVA_HOME=$(ls -d $MESOS_SANDBOX/jdk*/jre/); export JAVA_HOME=${JAVA_HOME%%/}; export PATH=$(ls -d $JAVA_HOME/bin):$PATH && export JAVA_OPTS=\\"-Xms256M -Xmx512M -XX:-HeapDumpOnOutOfMemoryError\\" && ./bootstrap -resolve=false -template=false && ./engine-scheduler/bin/engine ./engine-scheduler/svc.yml",
  "labels": {
